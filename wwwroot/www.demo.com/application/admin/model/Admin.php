...
class Admin extends Model
{
    ...
    public function add_admin($user_name, $password, $re_password)
    {
        ...
        $data['system_type'] = config('app.system_type');
    ...
    //导出操作日志
    public function export_operate_log($system_type = 1, $time = 1)
    {
        $map = [];
        $map[] = ['system_type', '=', $system_type];
        $map[] = ['is_delete', '=', 1];
        $admin_id_arr = Db::name('admin')->where($map)->column('aid');
        $map = [];
        $map[] = ['a.id', 'in', $admin_id_arr];
        $map[] = ['a.add_time', '>=', (time() - ($time * 86400))];
        $list = Db::name('operation')->alias('a')
        ->join('admin b', 'a.id = b.aid')
        ->field('a.*,b.aid,b.user_name')
        ->where($map)->order('op_id', 'asc')->select();
        // echo Db::name('operation')->getLastSql();die;
        $tileArr = [
            '管理员id',
            '管理员账号',
            '操作记录',
        ...
        foreach($list as $v) {
            if(strpos($v['url'],'admin/box/get_box_type_list')){
                ...
                $v['operate_name'] = '删除管理员';
            }else if(strpos($v['url'],'admin/group/get_group_list')){
                $v['operate_name'] = '群列表';
            }else if(strpos($v['url'],'admin/group/get_group_user_list')){
                $v['operate_name'] = '群成员列表';
            }else if(strpos($v['url'],'admin/group/add_group_user')){
                $v['operate_name'] = '群成员添加';
            }else if(strpos($v['url'],'admin/group/add_group')){
                $v['operate_name'] = '添加群';
            }else if(strpos($v['url'],'admin/group/edit_group')){
                $v['operate_name'] = '编辑群';
            }else if(strpos($v['url'],'admin/group/get_group_info')){
                $v['operate_name'] = '获取群信息';
            }else if(strpos($v['url'],'admin/group/del_group')){
                $v['operate_name'] = '删除群';
            }else if(strpos($v['url'],'admin/group/get_user_list')){
                $v['operate_name'] = '获取用列表';
            }else if(strpos($v['url'],'admin/group/get_group_file_list')){
                $v['operate_name'] = '获取群文件列表';
            }else if(strpos($v['url'],'admin/group/add_group_file')){
                $v['operate_name'] = '添加群文件';
            }else if(strpos($v['url'],'admin/group/del_group_file')){
                $v['operate_name'] = '删除群文件';
            }else if(strpos($v['url'],'admin/upload/file_upload')){
                $v['operate_name'] = '上传文件';
            }else if(strpos($v['url'],'admin/invitecode/get_group_list')){
                $v['operate_name'] = '邀请码列表';
            }else if(strpos($v['url'],'admin/invitecode/add_group')){
                $v['operate_name'] = '添加邀请码';
            }else if(strpos($v['url'],'admin/invitecode/del_group')){
                $v['operate_name'] = '删除邀请码';
            }else if(strpos($v['url'],'admin/invitecode/get_user_list')){
                $v['operate_name'] = '获取邀请用户';
            }else if(strpos($v['url'],'admin/upload/file_upload')){
                $v['operate_name'] = '上传文件';
            }else if(strpos($v['url'],'admin/Website/get_page')){
                $v['operate_name'] = '单页信息';
            }else if(strpos($v['url'],'admin/Website/page_list')){
                $v['operate_name'] = '单页列表';
            }else if(strpos($v['url'],'admin/Website/edit_page')){
                $v['operate_name'] = '修改单页';
            }else if(strpos($v['url'],'admin/Website/add_page')){
                $v['operate_name'] = '添加单页';
            } else if(strpos($v['url'],'admin/Statistics/welcome_data')){
                $v['operate_name'] = '获取首页统计数据';
            } else if(strpos($v['url'],'admin/common/check_login_status')){
                $v['operate_name'] = '检查登录状态';
            } else if(strpos($v['url'],'admin/common/get_menu_list')){
                $v['operate_name'] = '获取菜单列表';
            } else if(strpos($v['url'],'admin/user/user_real_name_list')){
                $v['operate_name'] = '用户实名列表';
            } else if(strpos($v['url'],'admin/user/get_user_info')){
                $v['operate_name'] = '用户信息';
            } else if(strpos($v['url'],'admin/user/get_user_info')){
                $v['operate_name'] = '用户信息';
            } else if(strpos($v['url'],'admin/user/get_user_info')){
            ...
            $temp = [
                ...
                '时间' => date('Y-m-d H:i:s', $v['add_time']),
            ];
            $data[] = $temp;
        }
        
        $filename = 'admin-operate.csv';
        $mark = '管理操作日志-' . date('YmdHis');
        ExportLib::zipData($data, $tileArr, $filename, $mark);
    }
