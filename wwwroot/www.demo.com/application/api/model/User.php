...
class User extends Model
{
    ...
    public function user_reg($user_name, $password, $reg_code)
    {
        ...
        if (!empty($reg_code)) {
            ...
            //     return ['code' => 201, 'msg' => '邀请码不正确', 'data' => null];
            // }
        } else {
        ...
        $data['head_pic'] = 'head_pic/head_pic1.png';
    ...
    public function user_reg_account($user_name, $password, $reg_code, $repassword)
    {
        ...
        if (!empty($reg_code)) {
            ...
            $invitecode = Db::name('invitecode')->where(['name' => $reg_code,'system_type'=>config('app.system_type')])->find();
            if(empty($invitecode)){
                return ['code' => 201, 'msg' => '邀请码不存在', 'data' => null];
            }
            // $map = [];
        ...
        try {
            ...
            if (!$reslut) {
             ...
             if (!empty($reg_code) && isset($invitecode)) {
                    $update = [];
                    //追加$uid到user_id,逗号分隔
                    if(empty($invitecode['user_id'])){
                        $update['user_id'] = $uid;
                    }else{
                        $update['user_id'] = $invitecode['user_id'].','.$uid;
                    }
                    //$update['status'] = 1;
                    $invitecode = Db::name('invitecode')->where('id',$invitecode['id'])->update($update);      
