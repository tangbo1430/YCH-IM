...
//         dump($reslut);die;
    ...
    //添加好友
    public function get_history($chat_type, $msg_time){
        $config = model('admin/Config')->get_system_config();
        $tencentyun_im_appid = $config['tencentyun_im_appid'];
        $im_admin = 'administrator';
        $admin_sig = $this->tencent_user_sig_info($im_admin);
        ...
        $postUrl = 'https://console.tim.qq.com/v4/open_msg_svc/get_history?sdkappid='.$tencentyun_im_appid.'&identifier='.$im_admin.'&usersig='.$admin_sig.'&random='.$rand.'&contenttype=json';
        // dump($friend_uid);
        $curlPost = array(
                'ChatType' => $chat_type,
                'MsgTime' => strval($msg_time)
            );
            // dump($curlPost);die;
        // dump($curlPost);die;    
        $curlPost = json_encode($curlPost);
        ...
        if($reslut['ActionStatus'] == 'OK'){
            ...
            return ['code' => 200, 'msg' => '可以添加', 'data' => $reslut['File'][0]['URL']];
