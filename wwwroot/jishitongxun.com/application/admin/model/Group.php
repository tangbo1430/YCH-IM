...
{
    //获取举报列表
    public function get_group_list($uid, $group_name, $order, $sort, $page = 1, $limit = 20)
    {
        $map = [];
        if (!empty($uid)) {
            $map[] = ['a.uid', '=', $uid];
        ...
        $map[] = ['a.is_delete', '=', 1];
        $map[] = ['a.system_type', '=', config('app.system_type')];
        if (!empty($group_name)) {
            $map[] = ['b.nick_name', 'like', '%' . $nick_name . '%'];
        }
        $list = db::name('group_v')
        ...
        $data = [];
        $data['count'] = db::name('group_v')->alias('a')->join('yy_user b', 'a.uid = b.uid')->where($map)->count();
        $data['list'] = $list;
    ...
    public function del_group($id)
    {
     ...
     //获取举报列表
    public function get_group_list_all( $order, $sort)
    {
        $map = [];
        if (!empty($uid)) {
            $map[] = ['a.uid', '=', $uid];
        ...
        $list = db::name('group_v')
            ->alias('a')->join('yy_user b', 'a.uid = b.uid')
            ->field('a.*,b.base64_nick_name,b.nick_name')->where($map)->order($order, $sort)->select();
        $return_data=[];
        foreach ($list as $k => $v) {
           ...
           $member_list=[];
            $has_group_uid = model('api/Tencent')->get_group_member_list($v['im_g_id'], 1, 1000);
            if($has_group_uid['code'] == 201) {
                ...
                $where=[];
                 $where[] = ['uid', 'in', $has_group_uid];
                 $user_list = Db::name('user')->where($where)->field('uid,nick_name,user_name')->select();
                 foreach($user_list as $key => $value) {
                    $mid_temp=[];
                    $mid_temp['uid']=$value['uid'];
                    $mid_temp['nick_name']=$value['nick_name'];
                    $mid_temp['user_name']=$value['user_name'];
                    $member_list[]=$mid_temp;
                 }
            }
    ...
    //群文件
    public function get_group_file_list($group_id, $file_name, $page = 1, $limit = 20)
    {
        $map = [];
        if($group_id) {
        ...
        if($file_name) {
         ...
         $data = [];
        $data['count'] = Db::name('group_file')->where($map)->count();  
        $data['list'] = Db::name('group_file')->where($map)
        ->page($page, $limit)
        ->select();
        return ['code' => 200, 'msg' => '获取成功', 'data' => $data];
    }
