...
function shuchu($value,$type = NULL,$file_name = 'cache.txt',$path =  ROOT_PATH.'runtime/'){
    if(empty($type)){
        file_put_contents($path.$file_name,date("Y-m-d H:i:s",time()).PHP_EOL.'--------Data start--------'.PHP_EOL.print_r($value,1).PHP_EOL.'--------Data end--------'.PHP_EOL,FILE_APPEND);
    }else{
        file_put_contents($path.$file_name,date("Y-m-d H:i:s",time()).PHP_EOL.'--------Data start--------'.PHP_EOL.print_r($value,1).PHP_EOL.'--------Data end--------'.PHP_EOL);
    }
}