...
class User extends Model
{
    ...
    public function edit_user_info($uid, $nick_name, $sex, $login_status)
    {
        ...
        $is_customer = input('is_customer', 2);
        

        $update_data = [];
        $update_data['uid'] = $user_info['uid'];
        $update_data['is_customer'] = $is_customer;
        ...
        if($update_data['base64_nick_name'] != $user_info['base64_nick_name']) {
            $im_data = [
                [
            ...
            model('api/Tencent')->modify_member_info($uid, $im_data);
        }
    ...
    public function add_no_check_user($filepath, $type)
    {
        if(empty($type)) {
            return ['code' => 201, 'msg' => "请选择操作类型", 'data' => null];
        }
        if(empty($filepath)) {
